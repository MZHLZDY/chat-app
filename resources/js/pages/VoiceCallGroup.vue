...
const acceptedParticipants = computed(() => participants.value.filter((p: Participant) => p.status === 'accepted'));
const rejectedParticipants = computed(() => participants.value.filter((p: Participant) => p.status === 'rejected' || p.status === 'ended' || p.status === 'left'));
const ringingParticipants = computed(() => participants.value.filter((p: Participant) => p.status === 'ringing' || p.status === 'calling'));
      ...
      <div v-else>
        ...
        <div v-if="isOutgoingCall">
          ...
          <div class="grid grid-cols-3 gap-2 mb-4 text-xs">
            <div class="bg-green-100 text-green-800 p-2 rounded"><div class="font-bold">{{ acceptedParticipants.length }}</div><div>Bergabung</div></div>
            <div class="bg-yellow-100 text-yellow-800 p-2 rounded"><div class="font-bold">{{ ringingParticipants.length }}</div><div>Memanggil</div></div>
            <div class="bg-red-100 text-red-800 p-2 rounded"><div class="font-bold">{{ rejectedParticipants.length }}</div><div>Menolak</div></div>
          </div>
        ...
        <div v-if="isOngoingCall">
          ...
          <div v-if="isCaller" class="grid grid-cols-3 gap-2 mb-3 text-xs">
            <div class="bg-green-100 text-green-800 p-2 rounded"><div class="font-bold">{{ acceptedParticipants.length }}</div><div>Bergabung</div></div>
            <div class="bg-yellow-100 text-yellow-800 p-2 rounded"><div class="font-bold">{{ ringingParticipants.length }}</div><div>Memanggil</div></div>
            <div class="bg-red-100 text-red-800 p-2 rounded"><div class="font-bold">{{ rejectedParticipants.length }}</div><div>Keluar</div></div>
          </div>
          ...
          <div class="max-h-32 overflow-y-auto mb-4 border-t border-b dark:border-gray-700">
            <div v-for="participant in participants" :key="participant.id" class="flex items-center justify-between p-2 text-sm">
              ...
              <div class="flex items-center gap-2">
                  <span :class="{ 'text-green-500': participant.status === 'accepted', 'text-yellow-500': participant.status === 'ringing' || participant.status === 'calling', 'text-red-500': participant.status === 'rejected' || participant.status === 'left' }" class="text-xs font-semibold">
                      {{ participant.status === 'accepted' ? '✓ Bergabung' : participant.status === 'ringing' ? '⌛ Berdering' : participant.status === 'calling' ? '⌛ Memanggil' : participant.status === 'rejected' ? 'Diabaikan' : participant.status === 'left' ? ' Keluar' : '' }}
                  </span>
                  <button v-if="isCaller && participant.status === 'left'" @click="recallParticipant(participant.id)" class="bg-green-500 text-white px-2 py-0.5 text-xs rounded-full hover:bg-gray-600"><Phone class="w-3 h-3"/></button>
              </div>
            </div>
          </div>
          
          <div class="flex justify-center gap-4">
              <button @click="toggleMute" :class="['p-3 rounded-full', isMuted ? 'bg-red-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-white']" title="Mute"><Mic class="w-5 h-5"/></button>
              <button v-if="isCaller" @click="endCall" class="bg-red-500 text-white p-3 rounded-full hover:bg-red-600 flex items-center justify-center gap-2 px-6" title="Bubarkan Panggilan"><PhoneForwarded class="w-5 h-5"/> <span>Bubarkan</span></button>
              <button v-else @click="leaveCall" class="bg-red-500 text-white p-3 rounded-full hover:bg-red-600 flex items-center justify-center gap-2 px-6" title="Keluar dari Panggilan"><PhoneForwarded class="w-5 h-5"/> <span>Keluar</span></button>
              <button @click="toggleSpeaker" :class="['p-3 rounded-full', isSpeakerOn ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-white']" title="Speaker"><Volume2 class="w-5 h-5"/></button>
          </div>
        </div>
